...
def build_invoice_payment_thank_you_text(
    *,
    ...
    lines.extend([
        "",
        "Благодарим за выбор нашей компании! Надеемся, вы остались довольны результатом.",
        "",
        "Если вам удобно, пожалуйста, поделитесь впечатлениями, оставив отзыв:",
        "",
        "Яндекс: https://yandex.ru/maps/org/10924053513/?add-review=true",
        "Google: https://g.page/r/CT-lPsP9hSHrEAE/review",
        "",
        "Можно просто поставить оценку или добавить пару слов: ваш отзыв поможет другим клиентам.",
    ])
    return "\n".join(lines)
...
def _build_attachment_file_name(
    ...
    *,
    ...
    return name or f"{_fallback_file_stem(attachment, index=index)}{extension}"
...
def _normalize_email_attachments(
    ...
    for idx, attachment in enumerate(attachments or [], start=1):
        ...
        file_name = _build_attachment_file_name(attachment, index=idx, file_content=file_content)
